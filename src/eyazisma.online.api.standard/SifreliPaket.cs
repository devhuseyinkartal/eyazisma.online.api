...
using CT_NihaiOzet = eyazisma.online.api.Api.V2X.CT_NihaiOzet;
...
namespace eyazisma.online.api
{
    ...
    public sealed class SifreliPaket : ISifreliPaket, IDisposable
    {
        ...
        /// </param>
        public ISifreliPaketOkuAction Versiyon1XIse(
        ...
        ///     List -> Şifreli pakete ait tüm doğrulama hatalarını belirtir.
        /// </param>
        public ISifreliPaketOkuAction Versiyon2XIse(
        ...
        public static ISifreliPaketOlustur Olustur(string sifreliPaketDosyaYolu)
        {
            if (string.IsNullOrWhiteSpace(sifreliPaketDosyaYolu))
                throw new ArgumentNullException(nameof(sifreliPaketDosyaYolu),
                    nameof(sifreliPaketDosyaYolu) + " boş olmamalıdır.");

            if (File.Exists(sifreliPaketDosyaYolu))
                try
                {
                    File.Delete(sifreliPaketDosyaYolu);
                }
                catch
                {
                }

            return new SifreliPaket(File.Open(sifreliPaketDosyaYolu, FileMode.OpenOrCreate, FileAccess.ReadWrite),
        ...
        public static ISifreliPaketOku Oku(Stream sifreliPaketStream)
        {
            if (sifreliPaketStream == null || sifreliPaketStream.Length == 0)
                throw new ArgumentNullException(nameof(sifreliPaketStream),
                    nameof(sifreliPaketStream) + " boş olmamalıdır.");
        ...
        public static ISifreliPaketOku Oku(string sifreliPaketDosyaYolu)
        {
            if (string.IsNullOrWhiteSpace(sifreliPaketDosyaYolu))
                throw new ArgumentNullException(nameof(sifreliPaketDosyaYolu),
                    nameof(sifreliPaketDosyaYolu) + " boş olmamalıdır.");
            if (!File.Exists(sifreliPaketDosyaYolu))
                throw new FileNotFoundException(nameof(sifreliPaketDosyaYolu),
                    "Verilen dosya yolu geçersizdir ya da dosyaya erişim yetkisi bulunmamaktadır.");

            return new SifreliPaket(File.Open(sifreliPaketDosyaYolu, FileMode.Open, FileAccess.Read),
                PaketModuTuru.Oku);
        }
    ...
    public sealed class SifreliPaketV1X : ISifreliPaketV1X
    {
            ...
            switch (paketModu)
            {
                case PaketModuTuru.Oku:
                {
                    ...
                    if (!_package.SifreliIcerikBilgisiExists())
                        _dogrulamaHatalari.Add(new DogrulamaHatasi
                        {
                            Hata = "E-Yazışma Paketi geçersizdir. \"" + nameof(Classes.SifreliIcerikBilgisi) +
                                   "\" bileşeni yoktur.",
                            HataTuru = DogrulamaHataTuru.Kritik
                        });
                        ...
                        try
                        {
                            var readedSifreliIcerikBilgisiUri = _package
                                .GetRelationshipsByType(Constants.RELATION_TYPE_SIFRELIICERIKBILGISI).First().TargetUri;
                            var readedSifreliIcerikBilgisi =
                                (CT_SifreliIcerikBilgisi)
                                new XmlSerializer(typeof(CT_SifreliIcerikBilgisi)).Deserialize(
                                    _package.GetPartStream(readedSifreliIcerikBilgisiUri));
        ...
        /// </summary>
        public bool SifreliIcerikBilgisiVarMi()
        {
            return _package.SifreliIcerikBilgisiExists();
        }
        ...
        {
            return _package.GetSifreliIcerikStream();
        }
        ...
        {
            return _package.GetSifreliIcerikDosyasiAdi();
        }
        ...
        public ISifreliPaketV1XOlusturPaketOzeti PaketOzetiEkle(Stream paketOzetiStream)
        {
            ...
            else
            {
                ...
                {
                    var readedPaketOzeti =
                        (CT_PaketOzeti) new XmlSerializer(typeof(CT_PaketOzeti)).Deserialize(paketOzetiStream);
                    PaketOzeti = readedPaketOzeti.ToPaketOzeti();
        ...
        public ISifreliPaketV1XOlusturBilesen BilesenleriOlustur()
        {
            ...
            SifreliIcerikBilgisi.URI = new List<string>
                {Constants.SIFRELEME_YONTEMI_URI_1, Constants.SIFRELEME_YONTEMI_URI_2};
            ...
            _package.PackageProperties.Revision = string.Format(Constants.PAKET_REVIZYON,
                Assembly.GetAssembly(typeof(SifreliPaket)).GetName().Version);
        ...
        /// </param>
        public ISifreliPaketV1XOkuBilesenAl BilesenleriAl(
            Action<bool, ISifreliPaketV1XOkuBilesen, List<DogrulamaHatasi>> bilesenAction)
        {
        ...
        public static ISifreliPaketV1XOlustur Olustur(string sifreliPaketDosyaYolu)
        {
            ...
            return new SifreliPaketV1X(File.Open(sifreliPaketDosyaYolu, FileMode.OpenOrCreate, FileAccess.ReadWrite),
                PaketModuTuru.Olustur);
        }
        ...
        public static ISifreliPaketV1XOku Oku(string sifreliPaketDosyaYolu)
        {
            ...
            if (!File.Exists(sifreliPaketDosyaYolu))
                throw new FileNotFoundException(nameof(sifreliPaketDosyaYolu),
                    "Verilen dosya yolu geçersizdir ya da dosyaya erişim yetkisi bulunmamaktadır.");
            return new SifreliPaketV1X(File.Open(sifreliPaketDosyaYolu, FileMode.Open, FileAccess.Read),
                PaketModuTuru.Oku);
        }
    ...
    public sealed class SifreliPaketV2X : ISifreliPaketV2X
    {
                    ...
                    if (_package.BelgeHedefUriExists())
                        try
                        {
                            var readedBelgeHedefUri = PackUriHelper.CreatePartUri(_package
                                .GetRelationshipsByType(Constants.RELATION_TYPE_BELGEHEDEF).First().TargetUri);
                            var readedBelgeHedef =
                                (Api.V2X.CT_BelgeHedef) new XmlSerializer(typeof(Api.V2X.CT_BelgeHedef)).Deserialize(
                                    _package.GetPartStream(readedBelgeHedefUri));
                            BelgeHedef = readedBelgeHedef.ToBelgeHedef();
                        }
                        catch (Exception ex)
                    ...
                    if (!_package.SifreliIcerikBilgisiExists())
                        ...
                        try
                        {
                            var readedSifreliIcerikBilgisiUri = _package
                                .GetRelationshipsByType(Constants.RELATION_TYPE_SIFRELIICERIKBILGISI).First().TargetUri;
                            var readedSifreliIcerikBilgisi =
                                (Api.V2X.CT_SifreliIcerikBilgisi) new XmlSerializer(
                                        typeof(Api.V2X.CT_SifreliIcerikBilgisi))
                                    .Deserialize(_package.GetPartStream(readedSifreliIcerikBilgisiUri));
                            SifreliIcerikBilgisi = readedSifreliIcerikBilgisi.ToSifreliIcerikBilgisi();
                        }
                        catch (Exception ex)
                        {
        ...
        public ISifreliPaketV2XOlusturNihaiOzet NihaiOzetEkle(Stream nihaiOzetStream)
        {
            ...
            else
            {
                ...
                {
                    var readedNihaiOzet =
                        (CT_NihaiOzet) new XmlSerializer(typeof(CT_NihaiOzet)).Deserialize(nihaiOzetStream);
                    NihaiOzet = readedNihaiOzet.ToNihaiOzet();
        ...
        ///     List -> Pakete ait tüm doğrulama hatalarını belirtir.
        /// </param>
        public ISifreliPaketV2XOkuBilesenAl BilesenleriAl(
            Action<bool, ISifreliPaketV2XOkuBilesen, List<DogrulamaHatasi>> bilesenAction)
        {
            bilesenAction.Invoke(KritikHataExists(), this, _dogrulamaHatalari);
            return this;
        ...
        public static ISifreliPaketV2XOlustur Olustur(string sifreliPaketDosyaYolu)
        {
            ...
            return new SifreliPaketV2X(File.Open(sifreliPaketDosyaYolu, FileMode.OpenOrCreate, FileAccess.ReadWrite),
                PaketModuTuru.Olustur);
        }
        ...
        public static ISifreliPaketV2XOku Oku(string sifreliPaketDosyaYolu)
        {
            ...
            if (!File.Exists(sifreliPaketDosyaYolu))
                throw new FileNotFoundException(nameof(sifreliPaketDosyaYolu),
                    "Verilen dosya yolu geçersizdir ya da dosyaya erişim yetkisi bulunmamaktadır.");
            return new SifreliPaketV2X(File.Open(sifreliPaketDosyaYolu, FileMode.Open, FileAccess.Read),
                PaketModuTuru.Oku);
        }
